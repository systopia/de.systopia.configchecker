...
class CRM_Configchecker_Form_Settings extends CRM_Core_Form {
  ...
  public function validate() {
    ...
    if (isset($values['send_notification_mail']) && $values['send_notification_mail']) {
      if (empty($values['check_config_notification_email'])) {
        $this->_errors['check_config_notification_email'] = ts("Please set a notification Email");
      } else {
        // validate email address
        if (!filter_var($values['check_config_notification_email'], FILTER_VALIDATE_EMAIL)) {
          $this->_errors['check_config_notification_email'] = ts("Please set a valid notification Email");
        }
      }
      if (empty($values['check_config_notification_from_email'])) {
        $this->_errors['check_config_notification_from_email'] = ts("Please set a notification Email Sender address");
      } else {
        // validate email address
        if (!filter_var($values['check_config_notification_from_email'], FILTER_VALIDATE_EMAIL)) {
          $this->_errors['check_config_notification_from_email'] = ts("Please set a valid notification Email Sender address");
        }
      }
    }
